...
function getCitiesNames(){
  ...
  .then((response)=>{
      ...
      for(let city of response.data.data){
        ...
        if(liElement.getAttribute("data-name")==firstCity){
...
function getCityPrayTime(name){
  ...
  let str = now.getDate()+"-"+(month<10?"0"+month:month)+"-"+now.getFullYear();
  console.log(str);
  let url = `https://api.aladhan.com/v1/timingsByCity/${str}?city=${name}&country=Tunisia&method=8`
  axios({
    method: 'get',
    url: url,
    responseType: 'json'
    ...
    .then(function (response) {
      ...
      document.querySelector(".date").innerHTML = weekDays[date.getDay()-1]+" "+date.getDate()+" "+monthNames[date.getMonth()]+","+date.getFullYear();
    });
}
